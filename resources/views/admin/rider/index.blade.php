@extends('admin.layout.master',['title' => 'View Riders'])
                    ...
                    <table id="zero-config" class="table table-striped dt-responsive nowrap" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $value->email }}</td>
                                <td>{{ $value->rider_code }}</td>
                                <td>{{ $value->avg_rating}}</td>
                                <td>{{ $value->dial_code.'-'.$value->phone }}</td>
                                <td><?php
                                    if ($value->image) {
                                        ?>
                                        <img src="{{url('uploads/rider/'.$value->image)}}" height="50" width="50">
                                    <?php } else { ?>
                                ...
                                <td class="table-action">
                                    <a href="{{action('Admin\riderController@edit',$value->id)}}" class=""><i class="far fa-edit"></i></a>

                                    <form action="{{action('Admin\riderController@destroy',$value->id)}}" id="form-<?= $value->id ?>" class="delete-form" method="post">
                                        {{csrf_field()}}
                                        @method('DELETE')
...
<!--                                           <a href="{{url('vehicletype/delete/'.$value->id)}}" class="ms-3"><i class="far fa-trash-alt"></i></a>-->
                                    ...
                                    <a href="{{url('admin/ride_rating?id='.$value->id.'&type=rider')}}" class="mb-3 rounded bs-tooltip"  title="View Ratings"><i class="fas fa-star"></i></a>
                                    <a href="{{url('admin/ride?id='.$value->id.'&type=rider')}}" class="mb-3 rounded bs-tooltip" title="View Rides"><i class="fas fa-car"></i></a>
