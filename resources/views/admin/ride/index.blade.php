<?php
if (($type == 'driver') && isset($id)) {
    $title =  'View ' . $driver_name . ' ( Driver ) Rides ';
} elseif (($type == 'rider') && isset($id)) {
    $title =  'View ' . $rider_name . ' ( Rider ) Rides ';
} else {
    $title =  'View Rides';
}
?>
                ...
                <div class="widget-content widget-content-area">
                                    ...
                                    <div id="myTable_filter" class="dataTables_filter d-flex align-items-start">
                                        ...
                                        <div class="mr-2">
                                            <select name="searchcolumn" class="form-control w-auto">
                                                ...
                                                <option value="ride_number" <?= ($searchcolumn == 'ride_number') ? 'selected' : '' ?>>Ride Number</option>
                                                <option value="ride_type_name" <?= ($searchcolumn == 'ride_type_name') ? 'selected' : '' ?>>Ride Type</option>
                    ...
                    <table id="zero-config" class="table table-striped dt-responsive nowrap" style="width:100%">
                        <thead>
                            ...
                            <tr>
                                <th> S.No. </th>
                                <th class="sorting" columnName="ride_number" onclick="setUrl('ride_number')">Ride Number</th>
                                <th class="sorting" columnName="ride_type_name" onclick="setUrl('ride_type_name')">Ride Type</th>
                                ...
                                <th class="sorting" columnName="start_location" onclick="setUrl('start_location')">PickUp Location</th>
                                <th class="sorting" columnName="end_location" onclick="setUrl('end_location')">Drop Location</th>
                                <th class="sorting" columnName="distance" onclick="setUrl('distance')">Distance</th>
                                <th class="sorting" columnName="duration" onclick="setUrl('duration')">Duration</th>
                                <th>Base Fare</th>
                                <th>Distance Fare</th>
                                <th>Time Fare</th>
                                <th>Tax</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= $i ?></td>
                                <td><?= $value->ride_number ?></td>
                                <td><?= $value->ride_type_name ?></td>
                                ...
                                <td><?= config('constant.currency_symbol') . $value->base_fare ?></td>
                                <td><?= config('constant.currency_symbol') . $value->distance_fare ?></td>
                                <td><?= config('constant.currency_symbol') . $value->time_fare ?></td>
                                <td><?= config('constant.currency_symbol') . $value->tax ?></td>
                                <td><?= config('constant.currency_symbol') . $value->total_bill ?></td>
                                <td><?= $value->ride_status_name ?></td>
                                <td><?= ($value->created_at) ? date('d M Y H:i:s', strtotime($value->created_at)) : ''; ?></td>

                                <td class="table-action">
                                    <a href="{{action('Admin\rideController@edit',$value->id)}}" class=""><i class="far fa-eye"></i></a>

                                    <form action="{{action('Admin\rideController@destroy',$value->id)}}" id="form-<?= $value->id ?>" class="delete-form" method="post">
                                        {{csrf_field()}}
                                        @method('DELETE')
                                    ...
                                    <a href="{{url('admin/ride_rating?id='.$value->id.'&type=ride')}}" class="mb-3 rounded bs-tooltip" title="View Ratings"><i class="fas fa-star"></i></a>
